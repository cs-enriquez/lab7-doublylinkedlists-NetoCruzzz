	...
	void StudentList::popBack() {
		if(head == nullptr){
			cout << "Error, List is Empty cannot proceed" << endl;
			return;
		}
		Node* current = head;
	...
	Student StudentList::retrieveStudent(int idNum) {
		...
		cout << "Student with ID" << idNum << " not found. Returning dummy student" << endl;
		return dummyStudent;
	}
	...
	cout << "Student with ID " << idNum << " not found. Nothing removed." << endl;
	}
	...
	void StudentList::updateGPA(int idNum, float newGPA) {
		...
		while (current != nullptr){
			if(current->data.id == idNum){
				...
				cout << "ID" << idNum << " has been updated with new GPA." << endl;
				return;
			}
			current = current->next;
		}
		cout << "Student with ID: " << idNum << " cannot be found. GPA not updated." << endl;
	}
